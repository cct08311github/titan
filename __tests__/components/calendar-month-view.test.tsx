...
describe("CalendarMonthView", () => {
  ...
  it("renders day cells for the current month", () => {
    ...
    // Should have day 1 and day 28 (minimum for any month)
  ...
  it("navigates to previous month", () => {
    ...
    const now = new Date();
    const prevBtn = screen.getByLabelText("上個月");
    fireEvent.click(prevBtn);

    // Month display should change
    const prevMonth = now.getMonth(); // 0-indexed, so current getMonth() = prev month in display
  ...
  it("renders manager team member selector", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText("選擇成員")).toBeInTheDocument();
    });
  });
