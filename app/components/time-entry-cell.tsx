...
const CATEGORIES = [
  ...
  { value: "ADMIN", label: "行政庶務", color: "bg-muted text-muted-foreground border-border" },
      ...
      <button
        ...
        className={cn(
          ...
          entry && entry.hours > 0
            ...
            : "border-dashed border-border text-muted-foreground hover:border-ring hover:text-foreground hover:bg-muted/30"
        )}
      >
        {entry && entry.hours > 0 ? (
        ...
        <div className="absolute top-full mt-1 left-1/2 -translate-x-1/2 z-50 w-56 bg-card border border-border rounded-xl shadow-2xl p-3 space-y-2.5">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-muted border border-border rounded-md px-2.5 py-1.5 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-ring cursor-pointer"
            >
              {CATEGORIES.map((c) => (
          ...
          <div>
            ...
            <input
              ...
              onKeyDown={(e) => e.key === "Enter" && handleSave()}
              className="w-full bg-muted border border-border rounded-md px-2.5 py-1.5 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
              placeholder="可選備註..."
            />
          </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex-1 bg-accent hover:bg-accent/80 text-foreground text-xs font-medium py-1.5 rounded-md transition-colors disabled:opacity-50"
            >
              {saving ? "儲存中..." : "儲存"}
            </button>
            {entry && (
