      ...
      <table className="w-full border-collapse text-sm">
          ...
          <tr className="border-b border-border">
            ...
            {DAYS.map((day, i) => (
              <th key={i} className="px-2 py-2.5 text-center min-w-[100px]">
                <div className="text-xs font-semibold text-foreground">週{day}</div>
                <div className="text-xs text-muted-foreground mt-0.5">{formatDateLabel(weekStart, i)}</div>
              </th>
            ))}
        ...
        <tbody>
          {taskRows.map((row, rowIdx) => (
            <tr key={row.taskId ?? `free-${rowIdx}`} className="border-b border-border/50 hover:bg-muted/30 transition-colors group">
              <td className="px-3 py-2 sticky left-0 bg-card group-hover:bg-muted/30 z-10">
              ...
              <td className="px-3 py-2 text-center">
                <span className={cn("text-xs font-medium tabular-nums", rowTotals[rowIdx] > 0 ? "text-foreground" : "text-muted-foreground")}>
        ...
        <tfoot>
          <tr className="border-t border-border bg-muted/50">
            <td className="px-3 py-2.5 sticky left-0 bg-muted z-10">
              <span className="text-xs font-semibold text-foreground">每日合計</span>
            </td>
            {dailyTotals.map((total, i) => (
              <td key={i} className="px-2 py-2.5 text-center">
                <span className={cn("text-xs font-semibold tabular-nums", total > 8 ? "text-amber-500" : total > 0 ? "text-emerald-500" : "text-muted-foreground")}>
