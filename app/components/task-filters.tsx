...
export function TaskFilters({ filters, onChange }: TaskFiltersProps) {
  ...
  const selectCls =
    "bg-card border border-border text-foreground text-sm rounded-md px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring cursor-pointer hover:border-ring transition-colors";
      ...
      {hasActiveFilters && (
        <button
          ...
          className={cn(
            ...
            "px-2 py-1.5 rounded-md border border-border hover:border-ring bg-card"
          )}
        >
          <X className="h-3 w-3" />
          清除篩選
