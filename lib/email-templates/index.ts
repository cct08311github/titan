...
export function taskAssignedEmail(taskTitle: string, taskId: string): { subject: string; html: string } {
  return {
    subject: `[TITAN] 您被指派了新任務：${taskTitle}`,
    html: `${WRAPPER_START}
      ...
      <a href="${BASE_URL}/kanban?taskId=${taskId}" style="display: inline-block; background: #4f46e5; color: #fff; padding: 10px 20px; border-radius: 6px; text-decoration: none; margin-top: 12px;">查看任務</a>
    ${WRAPPER_END}`,
  };
}
...
export function dueSoonEmail(taskTitle: string, taskId: string, dueDate: string): { subject: string; html: string } {
  return {
    subject: `[TITAN] 任務即將到期：${taskTitle}`,
    html: `${WRAPPER_START}
      ...
      <div style="background: #fffbeb; border-left: 4px solid #f59e0b; padding: 12px 16px; margin: 16px 0; border-radius: 4px;">
        ...
        <span style="color: #92400e;">到期日：${dueDate}</span>
      </div>
      <a href="${BASE_URL}/kanban?taskId=${taskId}" style="display: inline-block; background: #f59e0b; color: #fff; padding: 10px 20px; border-radius: 6px; text-decoration: none; margin-top: 12px;">查看任務</a>
    ${WRAPPER_END}`,
  };
}
...
export function overdueEmail(taskTitle: string, taskId: string, overdueDays: number): { subject: string; html: string } {
  return {
    subject: `[TITAN] 任務已逾期：${taskTitle}（逾期 ${overdueDays} 天）`,
    html: `${WRAPPER_START}
      ...
      <a href="${BASE_URL}/kanban?taskId=${taskId}" style="display: inline-block; background: #dc2626; color: #fff; padding: 10px 20px; border-radius: 6px; text-decoration: none; margin-top: 12px;">查看任務</a>
    ${WRAPPER_END}`,
  };
}
...
export function slaAlertEmail(taskTitle: string, taskId: string, timeLeft: string): { subject: string; html: string } {
  return {
    subject: `⚠️ [TITAN] SLA 即將到期：${taskTitle}（剩餘 ${timeLeft}）`,
    html: `${WRAPPER_START}
      ...
      <div style="background: #fef2f2; border: 2px solid #dc2626; padding: 16px; margin: 16px 0; border-radius: 8px;">
        <strong style="font-size: 18px;">${taskTitle}</strong><br/>
        <span style="color: #dc2626; font-size: 24px; font-weight: bold;">剩餘 ${timeLeft}</span>
      </div>
      <a href="${BASE_URL}/kanban?taskId=${taskId}" style="display: inline-block; background: #dc2626; color: #fff; padding: 12px 24px; border-radius: 6px; text-decoration: none; margin-top: 12px; font-size: 16px;">立即處理</a>
    ${WRAPPER_END}`,
  };
}
